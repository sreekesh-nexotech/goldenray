...
const FALLBACK_IMAGE = "/invertor image.png";

export default function CompareInverterCard({
  inverter,
}: {
  inverter: SolarInverter;
}) {
  const stars = STARS_BY_GRADE[inverter.overallRating] ?? 3;
  const chips = trustChips(inverter);
  const isTopPick = inverter.overallRating === "Excellent";
  ...
  return (
    <article className="flex h-full flex-col rounded-xl border border-[#E5E5EA] bg-white">
      ...
      <div className="relative h-[150px] sm:h-[180px]">
        <Image
          src={inverter.imageUrl || FALLBACK_IMAGE}
          alt={`${inverter.brand} ${inverter.name} ${capacity} ${inverter.type.toLowerCase()} inverter`}
          fill
