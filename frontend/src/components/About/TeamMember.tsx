...
export default function TeamMember({ name, title, description, linkedin, twitter, imageUrl }:TeamMemberProps){
  // Determine which social link to show (default to LinkedIn if both are provided)
  const socialLink = linkedin ? { platform: 'LinkedIn', url: linkedin, } 
  ...
  return (
    <div className="relative flex flex-col items-start text-left mb-4">
      <div className="w-24 h-24 rounded-full mb-4">
        {imageUrl && <Image src={imageUrl} alt={name} width={100} height={100} className="w-full h-full rounded-full object-cover" />}
      </div>
