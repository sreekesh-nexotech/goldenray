...
export default function UsageDetailsStep({
  ...
  const setelectronic_devices: React.Dispatch<
  ...
  const setelectric_vehicles: React.Dispatch<
  ...
  > = (updater) => {
    setFormData((prev) => ({
      ...
      electric_vehicles:
        typeof updater === "function"
          ? updater(prev.electric_vehicles)
          : updater,
    }));
  };
  ...
  return (
    <div className="space-y-12 p-0 md:p-6">
      ...
      <AdvanceVehicleManager
        ...
        validateInputs={(validator) =>
          (vehicleManagerValidator.current = validator)
        }
      />
