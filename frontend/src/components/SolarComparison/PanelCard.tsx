...
// Progress bar component with label-specific colors
function ProgressBar({ label, value }: { label: string; value: number }) {
  const barColor =
    label === "Heat Performance" || label === "Warranty"
      ? "#E6B13F"
      : "#1FA35B";
...
function KeralaClimateScoreBadge({ score }: { score: number }) {
  return (
    <div className="flex items-center gap-2 text-[10px] text-[#2F2F2F]">
      ...
      />
      <span className="font-semibold">Kerala Climate Score:</span>
  ...
  return (
    <div className="bg-white rounded-[20px] shadow-[0_6px_20px_rgba(0,0,0,0.04)] border border-[#E8E8E8] overflow-hidden hover:shadow-[0_10px_28px_rgba(0,0,0,0.06)] transition-shadow duration-300">
      ...
      <div className="px-4 sm:px-5 py-4 sm:py-5 space-y-3.5">
        ...
        {/* Name - Brand + Type + Wattage format */}
        ...
        {/* Type Badge */}
        <div>
          <span className="inline-flex items-center px-3 py-1 rounded-full text-[10px] font-medium bg-[#E7F1FF] text-[#1F6CD9]">
        ...
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[11px] text-[#3D3D3D]">
          <span>{panel.efficiency}% Efficiency</span>

          <span>{panel.wattage} Wp</span>

          <span>
            {panel.productWarranty} Yr - {panel.performanceWarranty} Yr Warranty
          </span>

          <span className="hidden sm:inline">
            {panel.temperatureCoefficient}%/°C
          </span>

          <span className="hidden sm:inline">{panel.ipRating}</span>
        </div>
        ...
        <div className="space-y-2 pt-1 pr-6 sm:pr-12 max-w-[90%]">
          <ProgressBar label="Efficiency" value={panel.ratings.efficiency} />
          <ProgressBar
            label="Heat Performance"
            value={panel.ratings.heatPerformance}
          />
          <ProgressBar label="Warranty" value={panel.ratings.warranty} />
          <ProgressBar
        ...
        <div className="pr-6 sm:pr-7">
          <KeralaClimateScoreBadge score={panel.keralaClimateScore} />
        </div>
