...
export default function FilterSidebar({
  ...
  const handlePanelTypeChange = (type: PanelType | "All") => {
    if (type === "All") {
      onFilterChange({ ...filters, panelTypes: [] });
    } else {
  ...
  const handleEfficiencyChange = (value: number, isMin: boolean) => {
    const newRange: [number, number] = isMin
      ...
      : [filters.efficiencyRange[0], value];
    onFilterChange({ ...filters, efficiencyRange: newRange });
  };
  ...
  const handleKeralaClimateToggle = () => {
    onFilterChange({
      ...filters,
      keralaClimateRated: !filters.keralaClimateRated,
    });
  };
  ...
  const hasActiveFilters =
    ...
    filters.efficiencyRange[0] !== 15 ||
    filters.efficiencyRange[1] !== 23 ||
    filters.warranties.productWarranty12Plus ||
    filters.warranties.performanceWarranty30 ||
    filters.brands.length > 0 ||
    filters.keralaClimateRated;
    ...
    <div className="space-y-4 text-[13px]">
          ...
          <div className="grid grid-cols-2 gap-2">
            {PANEL_TYPES.map((type) => {
              const isSelected =
                type.value === "All"
                  ? filters.panelTypes.length === 0
                  : filters.panelTypes.includes(type.value as PanelType);
              return (
                <button
                  key={type.value}
              ...
              <input
                ...
                style={{
                  background: `linear-gradient(to right, #074A4D ${
                    ((filters.efficiencyRange[0] - 15) / (23 - 15)) * 100
                  }%, #E5E7EB ${
                    ((filters.efficiencyRange[0] - 15) / (23 - 15)) * 100
                  }%)`,
                }}
      ...
      <div className="border-b border-gray-200 pb-3">
        <button
          ...
          className="flex items-center justify-between w-full py-1.5 mb-2"
        >
          <span className="font-semibold text-sm text-gray-900">Warranty</span>
        ...
        {expandedSections.warranty && (
          <div className="space-y-2.5">
            <label className="flex items-center gap-2.5 cursor-pointer group">
              <input
                type="checkbox"
                checked={filters.warranties.productWarranty12Plus}
            ...
            <label className="flex items-center gap-2.5 cursor-pointer group">
              <input
                type="checkbox"
                checked={filters.warranties.performanceWarranty30}
      ...
      <button
        ...
        style={{
          background: filters.keralaClimateRated
  ...
  return (
    <>
      ...
      <aside className="hidden lg:block w-64 xl:w-72 flex-shrink-0">
        <div className="sticky top-24 bg-white rounded-2xl p-4 shadow-sm border border-gray-100 max-h-[calc(100vh-7rem)] overflow-y-auto">
